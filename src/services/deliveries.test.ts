...
/* ---------- ★ P4.3f: 단위 폴백 체인 (입고·출고 공통 — receipts.test.ts 미러) ---------- */
/*  수주 라인 unit 이 null 이면 products.unit 대신 'PCS' 가 원장에 박히던 결함의 교정.
    한쪽만 고치면 GR·DLV 두 원장이 어긋난다 → docFlow 단일 진실을 양쪽이 재수출. */

describe("resolveUom — 라인 uom → products.unit, 'PCS' 발명 금지", () => {
  it("①라인에 단위가 있으면 라인 값", () => {
    expect(resolveUom("M", "EA")).toBe("M");
  });

  it("②라인이 비면 품목 마스터 unit 으로", () => {
...
describe("resolveUomOrThrow — ③둘 다 없으면 저장 거부 (한국어 안내)", () => {
  it("★둘 다 없으면 품목명을 담아 한국어로 거부한다", () => {
    expect(() => resolveUomOrThrow(null, null, "케이블")).toThrowError(/단위/);
    expect(() => resolveUomOrThrow(null, null, "케이블")).toThrowError(
      /케이블/,
    );
  });

  it("해석되면 통과", () => {
    expect(resolveUomOrThrow(null, "M", "케이블")).toBe("M");
  });
});

describe("resolveDocLineUom — 출고측 미러 (규칙은 docFlow 단일 진실)", () => {
  it("수주 라인 행 없음·품목 미연결·마스터 단절은 null (RPC 정확한 에러에 양보)", () => {
    expect(resolveDocLineUom(undefined, new Map(), "케이블")).toBeNull();
    expect(
      resolveDocLineUom(
    ...
    expect(
      resolveDocLineUom(
        { unit: null, productId: "P-없음", productName: "케이블" },
        new Map(),
        null,
      ),
    ).toBeNull();
  ...
  it("라인 unit → 마스터 unit → (마스터 행은 있는데 unit 없음) 거부", () => {
    const m = new Map<string, string | null>([
      ["P1", "M"],
      ["P2", null],
    ]);
    expect(
      resolveDocLineUom({ unit: "EA", productId: "P1", productName: "케이블" }, m, null),
    ).toBe("EA");
    expect(
      resolveDocLineUom({ unit: null, productId: "P1", productName: "케이블" }, m, null),
    ).toBe("M");
    expect(() =>
      resolveDocLineUom({ unit: null, productId: "P2", productName: "케이블" }, m, null),
    ).toThrowError(/단위/);
  });

  it("★P4.4h 클라-서버 단위 불일치는 저장 거부 — 입고와 같은 규칙(단일 진실 재수출)", () => {
    // save_delivery 도 수주 라인 uom → products.unit 로 서버가 재해석하고,
    // 클라 uom 은 제공 시 일치해야 한다. 규칙이 GR 과 갈라지면 두 원장이 어긋난다.
    expect(uomConflict("KG", "EA")).toBe(true);
    expect(uomConflict(null, "EA")).toBe(false);
    expect(uomConflict(" EA ", "EA")).toBe(false);
  });
});
