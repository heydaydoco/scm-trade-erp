...
/* ---------- ⑧ P5.3 판정 ② — 문서 헤더 Container No. 3상태 ----------
 *
 * containers_snapshot 의 세 상태를 그대로 표시 규칙으로 옮긴다:
 *   NULL(P5.3 이전 발행) → 기존 container_no 스칼라 폴백(재인쇄 불변 계약)
 *   빈 구조(적입 스코프 0건) → 항목 생략. **스칼라 폴백 발동 금지**
 *   값 존재 → 스냅샷 번호 조인(", "), 번호 미확정은 TBA
 *
 * 소비처 3곳(CI 인쇄·PL 인쇄·무역서류 상세)이 이 함수 하나만 쓴다.
 */

describe("documentContainerNoLabel — 스냅샷 3상태 × TBA", () => {
  it("★NULL(P5.3 이전 발행)은 기존 스칼라를 그대로 낸다 — 재인쇄 불변", () => {
    expect(documentContainerNoLabel(null, "MSKU1234567")).toBe("MSKU1234567");
  });

  it("★NULL 이고 스칼라도 없으면 항목 자체가 없다", () => {
    expect(documentContainerNoLabel(null, null)).toBeNull();
  });

  it("★빈 구조는 항목 생략 — 스칼라로 폴백하지 않는다(신규 발행의 사실은 스냅샷)", () => {
    expect(documentContainerNoLabel([], "MSKU1234567")).toBeNull();
    expect(documentContainerNoLabel([], null)).toBeNull();
  });

  it("★번호가 여럿이면 ', ' 로 잇는다", () => {
    expect(documentContainerNoLabel(["MSKU1234567", "TGHU7654321"], null)).toBe(
      "MSKU1234567, TGHU7654321",
    );
  });

  it("★번호 미확정 컨테이너는 TBA 로 자리를 지킨다(칸이 사라지지 않는다)", () => {
    expect(documentContainerNoLabel([null], null)).toBe("TBA");
    expect(documentContainerNoLabel(["MSKU1234567", null], null)).toBe(
      "MSKU1234567, TBA",
    );
    expect(documentContainerNoLabel(["", "  ", "TGHU7654321"], null)).toBe(
      "TBA, TBA, TGHU7654321",
    );
  });

  it("★값이 있으면 스칼라는 무시된다(레거시 값이 신규 사실을 오염시키지 않는다)", () => {
    expect(documentContainerNoLabel(["MSKU1234567"], "OLD-LEGACY-NO")).toBe(
      "MSKU1234567",
    );
  });
});
