...
/* ---------- 포커스 앵커: 하류 주문이 없는 초점도 자신을 렌더 ---------- */

describe("buildDocChain — 하류 주문이 없어도 초점 노드는 반드시 나온다", () => {
  it("★견적 초점(파생 수주 없음) → 견적·문의 노드가 나온다", async () => {
    const fx = baseFixture();
    fx.quotations.push({ id: "Q2", num: "QT-202606-002", date: "2026-06-03", status: "sent", inquiryId: "INQ2" });
    fx.inquiries.push({ id: "INQ2", date: "2026-06-02", status: "reviewing" });
    const { repo } = makeFake(fx);
    const res = await buildDocChain({ type: "quotation", id: "Q2" }, repo);
    expect(res.found).toBe(true);
    const keys = res.chain!.nodes.map((n) => n.key);
    expect(keys).toContain("quotation:Q2");
    expect(keys).toContain("inquiry:INQ2");
    expect(res.chain!.nodes.find((n) => n.key === "quotation:Q2")?.focus).toBe(true);
  });

  it("★선적 초점(연결 주문 없음) → 선적 노드가 나온다", async () => {
    const fx = baseFixture();
    fx.shipments.push({ id: "SH_EMPTY", num: "SHP-202607-009", date: "2026-07-09", status: "draft" });
    const { repo } = makeFake(fx);
    const res = await buildDocChain({ type: "shipment", id: "SH_EMPTY" }, repo);
    expect(res.found).toBe(true);
    expect(res.chain!.nodes.find((n) => n.key === "shipment:SH_EMPTY")?.focus).toBe(true);
  });
});
