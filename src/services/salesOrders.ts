...
interface SalesOrderLineRow {
  ...
  ref_quotation_line_id: string | null;
...
function computeTotals(input: SalesOrderInput): {
  subtotal: number;
  total: number;
} {
...
function mapLineRow(row: SalesOrderLineRow): SalesOrderLine {
  ...
  return {
    ...
    refQuotationLineId: row.ref_quotation_line_id,
  };
}

/**
 * 헤더 행 + (선택)라인 행 → 도메인 SalesOrder.
 * 라인이 주어지면 합계를 라인에서 재계산(원칙 2), 없으면(목록) 저장된 스냅샷 사용.
 */
function assembleSalesOrder(
  row: SalesOrderRow,
  lineRows: SalesOrderLineRow[] | null,
): SalesOrder {
  ...
  return {
    ...
    refQuotationId: row.ref_quotation_id,
...
/** 도메인 입력 → save_sales_order RPC용 헤더 jsonb (번호·updated_at은 RPC가 채움). */
function headerPayload(
  input: SalesOrderInput,
  totals: { subtotal: number; total: number },
): Record<string, unknown> {
  return {
    ref_quotation_id: input.refQuotationId,
...
/** 도메인 라인 → save_sales_order RPC용 라인 jsonb 배열 (amount는 서버 계산값). */
function linePayload(
  ...
  return lines.map((l, i) => ({
    ...
    ref_quotation_line_id: l.refQuotationLineId,
    sort_order: i,
  }));
}

/**
 * 견적 → 수주 드래프트 (원칙 3 — 참조 생성).
 * 견적 데이터를 복사해 수주 입력 초안을 만든다(저장 X). 각 라인은 출처 견적 라인(refQuotationLineId)을 기억.
 */
export function buildSalesOrderDraftFromQuotation(q: Quotation): SalesOrderInput {
  return {
    refQuotationId: q.id,
    ...
    destinationAirport: q.destinationAirport,
    transport: q.transport,
    discount: q.discount,
    status: "draft",
    notes: q.notes,
    termsConditions: q.termsConditions ?? DEFAULT_SO_TERMS,
    lines: q.lines.map((l) => ({
...
/**
 * 수주 저장 — 번호발번 + 헤더 + 라인을 단일 트랜잭션 함수(save_sales_order)로 원자 처리.
 * 중간 실패 시 전부 롤백(데이터 손실·유령전표·번호 결번 없음).
 * 참조: db/migrations/p2.2_sales_orders.sql, p1.1_doc_numbering.sql
 * p_id=null이면 등록(함수가 next_doc_number로 'sales_order' 채번), 있으면 수정(번호 불변).
 */
async function saveViaRpc(
  ...
  if (error) throw new Error(`수주 저장 실패: ${error.message}`);
  const result = data as { id?: string } | null;
  if (!result?.id) throw new Error("수주 저장 결과가 올바르지 않습니다.");
  return result.id;
}

/** 수주 목록 (최신순). 합계는 저장된 스냅샷 사용(저장 시 라인에서 재계산됨). */
export async function listSalesOrders(): Promise<SalesOrder[]> {
  ...
  if (error) throw new Error(`수주 목록 조회 실패: ${error.message}`);
  const rows = (data ?? []) as unknown as SalesOrderRow[];
  return rows.map((r) => assembleSalesOrder(r, null));
}

/** 수주 1건 조회 (라인 포함, 합계는 라인에서 재계산). 없으면 null. */
export async function getSalesOrder(id: string): Promise<SalesOrder | null> {
  ...
  const { data, error } = await supabase
    ...
    .eq("id", id)
    .maybeSingle();
  if (error) throw new Error(`수주 조회 실패: ${error.message}`);
  if (!data) return null;
  ...
  if (lineErr) throw new Error(`수주 품목 조회 실패: ${lineErr.message}`);

  return assembleSalesOrder(
    data as unknown as SalesOrderRow,
    (lineData ?? []) as unknown as SalesOrderLineRow[],
  );
}

/** 수주 등록 — save_sales_order 트랜잭션으로 번호+헤더+라인 원자 저장. */
export async function createSalesOrder(
  ...
  const created = await getSalesOrder(id);
  if (!created) throw new Error("수주 등록 후 조회에 실패했습니다.");
  return created;
}

/**
 * 수주 수정 (정정). 번호는 불변(원칙 6). 헤더+라인 교체를 단일 트랜잭션으로(원자성).
...
export async function updateSalesOrder(
  ...
  const updated = await getSalesOrder(id);
  if (!updated) throw new Error("수주 수정 후 조회에 실패했습니다.");
  return updated;
}
