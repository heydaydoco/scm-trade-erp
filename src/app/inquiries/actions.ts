"use server";

import { revalidatePath } from "next/cache";
import { redirect } from "next/navigation";
import { createInquiry, updateInquiry } from "@/services/inquiries";
import type { InquiryInput } from "@/services/types";

export interface InquiryFormState {
  error?: string;
  /** 에러 시 입력값을 폼에 되돌려 재시드(데이터 유실 방지). */
  values?: Record<string, string>;
}

const ECHO_FIELDS = [
  "partnerId",
  "inquiryDate",
  "productId",
  "productName",
  "hsCode",
  "quantity",
  "unit",
  "transport",
  "destinationCountry",
  "destinationPort",
  "destinationAirport",
  "incoterms",
  "paymentTerms",
  "requiredDeliveryDate",
  "status",
  "notes",
];

const CHECKBOX_FIELDS = ["sampleRequested", "ndaRequired"];

/** 제출된 원본 값들을 모아 에러 시 폼 재시드에 사용. */
function collectValues(formData: FormData): Record<string, string> {
  const values: Record<string, string> = {};
  for (const f of ECHO_FIELDS) {
    const v = formData.get(f);
    if (typeof v === "string") values[f] = v;
  }
  for (const cb of CHECKBOX_FIELDS) {
    values[cb] = formData.get(cb) === "on" ? "on" : "";
  }
  return values;
}

function parseInquiryForm(formData: FormData): InquiryInput {
  const get = (k: string) => {
    const v = formData.get(k);
    return typeof v === "string" && v.trim() !== "" ? v.trim() : null;
  };

  const partnerId = get("partnerId");
  if (!partnerId) throw new Error("거래처를 선택하세요.");

  const productName = get("productName");
  if (!productName) throw new Error("품목명을 입력하세요.");

  const inquiryDate = get("inquiryDate");
  if (!inquiryDate) throw new Error("접수일을 입력하세요.");

  let quantity: number | null = null;
  const qtyRaw = get("quantity");
  if (qtyRaw !== null) {
    const parsed = Number(qtyRaw.replace(/,/g, ""));
    if (!Number.isFinite(parsed) || parsed < 0) {
      throw new Error("수량은 0 이상 숫자로 입력하세요.");
    }
    quantity = parsed;
  }

  return {
    partnerId,
    inquiryDate,
    productId: get("productId"),
    productName,
    hsCode: get("hsCode"),
    quantity,
    unit: get("unit"),
    transport: get("transport"),
    destinationCountry: get("destinationCountry"),
    destinationPort: get("destinationPort"),
    destinationAirport: get("destinationAirport"),
    incoterms: get("incoterms"),
    paymentTerms: get("paymentTerms"),
    requiredDeliveryDate: get("requiredDeliveryDate"),
    sampleRequested: formData.get("sampleRequested") === "on",
    ndaRequired: formData.get("ndaRequired") === "on",
    status: get("status") ?? "received",
    notes: get("notes"),
  };
}

/**
 * 문의 등록/수정 Server Action (등록·수정 겸용; id 있으면 수정).
 * 검증/저장 실패 시 입력값을 함께 돌려주고(values), 성공 시 목록으로 이동한다.
 */
export async function saveInquiryAction(
  _prev: InquiryFormState,
  formData: FormData,
): Promise<InquiryFormState> {
  const values = collectValues(formData);

  let input: InquiryInput;
  try {
    input = parseInquiryForm(formData);
  } catch (e) {
    return {
      error: e instanceof Error ? e.message : "입력값을 확인해주세요.",
      values,
    };
  }

  const id = formData.get("id");
  try {
    if (typeof id === "string" && id) {
      await updateInquiry(id, input);
    } else {
      await createInquiry(input);
    }
  } catch (e) {
    return {
      error: e instanceof Error ? e.message : "저장에 실패했습니다.",
      values,
    };
  }

  // 성공: 목록 캐시 갱신 후 이동 (redirect는 try 밖 — NEXT_REDIRECT를 삼키지 않게).
  revalidatePath("/inquiries");
  redirect("/inquiries");
}
