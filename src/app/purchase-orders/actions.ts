...
function parseLines(raw: FormDataEntryValue | null): PurchaseOrderLineInput[] {
  ...
  for (const item of parsed) {
    ...
    if (!Number.isFinite(unitPrice) || unitPrice < 0) {
      throw new Error(`'${productName}'의 매입단가를 0 이상 숫자로 입력하세요.`);
    }
    ...
    lines.push({
      ...
      productName,
      hsCode:
        typeof r.hsCode === "string" && r.hsCode.trim() ? r.hsCode.trim() : null,
      description:
      ...
      unitPrice,
      refSoLineId:
        typeof r.refSoLineId === "string" && r.refSoLineId
          ? r.refSoLineId
          : null,
    });
  }
...
function parsePurchaseOrderForm(formData: FormData): PurchaseOrderInput {
  const get = (k: string) => {
  ...
  if (!partnerId) throw new Error("공급사를 선택하세요.");
  ...
  return {
    refSalesOrderId: get("refSalesOrderId"),
    partnerId,
    orderDate,
...
/**
 * 발주 등록/수정 Server Action (등록·수정 겸용; id 있으면 수정).
...
export async function savePurchaseOrderAction(
  ...
  try {
    input = parsePurchaseOrderForm(formData);
  } catch (e) {
    return {
