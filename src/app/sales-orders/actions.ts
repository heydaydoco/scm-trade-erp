...
import { createSalesOrder, updateSalesOrder } from "@/services/salesOrders";
import type { SalesOrderInput, SalesOrderLineInput } from "@/services/types";
...
function parseLines(raw: FormDataEntryValue | null): SalesOrderLineInput[] {
  ...
  for (const item of parsed) {
    ...
    lines.push({
      ...
      unitPrice,
      refQuotationLineId:
        typeof r.refQuotationLineId === "string" && r.refQuotationLineId
          ? r.refQuotationLineId
          : null,
    });
  }
  return lines;
...
function parseSalesOrderForm(formData: FormData): SalesOrderInput {
  const get = (k: string) => {
  ...
  if (!orderDate) throw new Error("주문일을 입력하세요.");
  ...
  return {
    refQuotationId: get("refQuotationId"),
    partnerId,
    orderDate,
    requestedDeliveryDate: get("requestedDeliveryDate"),
    currency: get("currency"),
    exchangeRate,
    fxSource: get("fxSource"),
    fxQuotedAt: get("fxQuotedAt"),
...
/**
 * 수주 등록/수정 Server Action (등록·수정 겸용; id 있으면 수정).
...
export async function saveSalesOrderAction(
  ...
  try {
    input = parseSalesOrderForm(formData);
  } catch (e) {
    return {
