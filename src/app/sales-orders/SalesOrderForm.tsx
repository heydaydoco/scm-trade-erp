...
interface LineRow {
  ...
  quantity: string;
  unitPrice: string;
  refQuotationLineId: string; // 참조 출처 견적 라인(있으면 유지, 새 행은 빈 값)
}
...
export function SalesOrderForm({
  ...
  // 라인 초기값: 수정(salesOrder) > 참조생성(draft) > 빈 1행.
  // 초기 key는 결정적(ln-init-*) — 렌더 중 ref 접근 금지.
  const [lines, setLines] = useState<LineRow[]>(() => {
    const src = salesOrder?.lines?.length
      ? salesOrder.lines
      : draft?.lines?.length
        ? draft.lines
        : null;
    ...
    return src.map((l, i) => ({
      ...
      refQuotationLineId: l.refQuotationLineId ?? "",
    }));
  });
  ...
  // 서버로 보낼 라인 JSON (빈 라인 제외; amount는 서버가 계산; 참조 견적 라인 보존)
  const linesPayload = JSON.stringify(
    lines
      ...
      .map((l) => ({
        ...
        refQuotationLineId: l.refQuotationLineId || null,
      })),
  );

  const partnerDefault = v?.partnerId ?? salesOrder?.partnerId ?? draft?.partnerId ?? "";
  const partnerMissing =
    !!partnerDefault && !partners.some((p) => p.id === partnerDefault);

  return (
    <form action={formAction} className="space-y-6">
      {salesOrder ? <input type="hidden" name="id" value={salesOrder.id} /> : null}
      <input
        type="hidden"
        name="refQuotationId"
        value={
      ...
      {salesOrder ? (
        ...
        <p className="text-sm text-zinc-400">
          수주번호는 저장 시 자동 발번됩니다 (SO-YYYYMM-NNN).
          {draft?.refQuotationId ? " · 견적에서 참조 생성됨" : ""}
        </p>
      )}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          >
            ...
            {partnerMissing ? (
              <option value={partnerDefault}>
                {salesOrder?.partnerName ?? partnerDefault} (기존 값)
              </option>
            ) : null}
            {partners.map((p) => (
        ...
        <Field label="주문일" required>
          <input
            ...
            type="date"
            className={inputClass}
            defaultValue={v?.orderDate ?? salesOrder?.orderDate ?? defaultDate}
            required
          />
        </Field>
        <Field label="납기 요청일">
        ...
        <Field label="상태">
          <select
            name="status"
            className={inputClass}
            defaultValue={v?.status ?? salesOrder?.status ?? draft?.status ?? "draft"}
          >
            {SO_STATUS.map((s) => (
        ...
        <Field label="인코텀즈">
          <select
            name="incoterms"
            className={inputClass}
            defaultValue={v?.incoterms ?? salesOrder?.incoterms ?? draft?.incoterms ?? ""}
          >
        ...
        <Field label="결제조건">
          <select
            ...
            className={inputClass}
            defaultValue={
              v?.paymentTerms ?? salesOrder?.paymentTerms ?? draft?.paymentTerms ?? ""
            }
          >
            <option value="">선택 안 함</option>
            {PAYMENT_TERMS.map((c) => (
        ...
        <Field label="운송 방법">
          <select
            name="transport"
            className={inputClass}
            defaultValue={v?.transport ?? salesOrder?.transport ?? draft?.transport ?? ""}
          >
            <option value="">선택 안 함</option>
            {TRANSPORT.map((t) => (
        ...
        <Field label="목적지 국가">
          <input
            ...
            className={inputClass}
            defaultValue={
              v?.destinationCountry ?? salesOrder?.destinationCountry ?? draft?.destinationCountry ?? ""
            }
          />
        </Field>
        <div className="hidden sm:block" />
        <Field label="목적지 항구">
          <input
            ...
            className={inputClass}
            defaultValue={
              v?.destinationPort ?? salesOrder?.destinationPort ?? draft?.destinationPort ?? ""
            }
          />
        </Field>
        <Field label="목적지 공항">
          <input
            ...
            className={inputClass}
            defaultValue={
              v?.destinationAirport ?? salesOrder?.destinationAirport ?? draft?.destinationAirport ?? ""
            }
          />
        </Field>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <Field label="비고 (Notes)" full>
          <textarea
            ...
            rows={2}
            className={inputClass}
            defaultValue={v?.notes ?? salesOrder?.notes ?? draft?.notes ?? ""}
          />
        </Field>
      </div>
      ...
      <div className="flex gap-3 pt-2">
        ...
        >
          {pending ? "저장 중…" : salesOrder ? "수정 저장" : "등록"}
        </button>
        <Link
          href="/sales-orders"
