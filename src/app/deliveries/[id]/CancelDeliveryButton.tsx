...
import { cancelDeliveryAction, type DeliveryFormState } from "../actions";

/**
 * 출고 취소 — 삭제가 아니라 상태 + **원장 역분개**(원칙 1·5).
 * 원장 행은 지워지지 않고 반대부호 REVERSAL 이 쌓인다. 수주 잠금도 함께 풀린다.
  ...
  return (
    <form action={formAction} className="space-y-2 rounded-md border border-red-200 bg-red-50 p-3">
      ...
      <p className="text-sm text-red-900">
        <b>{deliveryNo}</b> 출고를 취소합니다. 재고가 역분개로 원복되고(다시 늘어납니다),
        이 출고가 잠그고 있던 수주의 잠금이 풀립니다. 기록은 지워지지 않고 &apos;취소&apos;
        상태로 남습니다.
      </p>
      <input
        name="memo"
