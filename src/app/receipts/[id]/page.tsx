...
export default async function ReceiptDetailPage({
  ...
  // 이 입고가 전기한 원장 행(취소했다면 REVERSAL 도 함께 보인다 — 사실을 그대로).
  ...
  return (
    <div className="mx-auto max-w-5xl px-8 py-8">
      <div className="mb-6 flex items-start justify-between gap-4">
        <PageHeader title="입고 상세" subtitle={receipt.grNo} />
        <span
          className={`rounded px-2 py-1 text-sm ${
            cancelled ? "bg-slate-100 text-slate-500" : "bg-emerald-50 text-emerald-700"
          }`}
        >
          {labelOf(GR_STATUS, receipt.status)}
        </span>
      </div>

      <div className="mb-4 flex flex-wrap gap-4 text-sm">
        <Link
          href={`/purchase-orders/${receipt.refDocId}`}
          className="text-violet-700 hover:underline"
        >
          📄 출처 발주 보기 ({receipt.poNumber ?? "—"}) →
        </Link>
      ...
      <dl className="mb-6 grid grid-cols-2 gap-3 rounded-lg border border-slate-200 p-4 text-sm sm:grid-cols-4">
        ...
        <div>
          ...
          <dd>{receipt.warehouseCode}</dd>
        </div>
        <div>
          <dt className="text-xs text-slate-500">공급사</dt>
          <dd>{receipt.partnerName ?? "—"}</dd>
        </div>
        <div>
        ...
        {receipt.memo && (
          <div className="col-span-2 sm:col-span-4">
            ...
            <dd className="text-slate-700">{receipt.memo}</dd>
          </div>
        )}
      </dl>

      <h2 className="mb-2 text-sm font-semibold text-slate-900">입고 품목</h2>
            ...
            {movements.map((m) => (
              <tr key={m.id}>
                ...
                <td className="px-4 py-2">
                  ...
                  >
                    {m.movementType === "REVERSAL" ? "역분개" : "구매 입고"}
                  </span>
                </td>
      ...
      <p className="mb-6 text-xs text-slate-500">
        {cancelled
          ? "취소된 입고입니다 — 원장의 입고 행은 지워지지 않고, 반대부호 역분개 행이 추가되어 재고가 원복되었습니다(원칙 1)."
          : "입고 저장과 원장 전기는 한 트랜잭션입니다 — 입고만 남거나 원장만 남는 상태가 존재할 수 없습니다."}
      </p>

      {!cancelled && (
        <CancelReceiptButton receiptId={receipt.id} grNo={receipt.grNo} />
      )}
    </div>
  );
}
