...
- **P4 — 재고 원장 + 출고/입고 + 서류생성 + 문서흐름**: 재고 코어, Delivery/GR(참조생성), CI/PL 생성기, 흐름 추적 화면 ← **여기까지가 진짜 ERP의 1차 완성**
  ...
  - ✅ **P4.4 선적 화물·당사자 스냅샷·S/I 완료 = P4.4 종결** (2026-07-17, 커밋 86e3818~a6bab2a) — `shipment_lines`(주문라인 소프트 포인터가 잔량·가드의 축. **원장 전기 없음** — 선적은 물류 전표, 재고 이동은 GR/DLV 만. 금액·환율 없음 — P4 수량 전용) + `shipment_parties`(shipper/consignee/notify, `unique(shipment_id, role)` — **인쇄는 이 스냅샷만 본다**: 거래처 마스터를 나중에 고쳐도 과거 서류가 소급 변경되지 않는다) + `shipments.shipping_marks`. 승인된 잠금 예외 3건 적용: `ship_number` UNIQUE(중복 0)·status/direction CHECK(위반 0).
    ...
    - **남은 백로그**: ① H3 잔존 — anon 직접 REST 호출은 여전히 RPC 의 'PCS' 폴백에 닿는다(마이그레이션 사안) ② `save_stock_adjustment` 의 `coalesce(unit,'PCS')` 단위 발명(제3의 원장 쓰기 경로) ③ 구 선적 3테이블·`doc_counters` 봉인 공백 — **P4.4h** 에서 감사표 기반 결정 ④ 발번 999건/월 상한(lpad 절단) ⑤ `so_number`·`po_number` UNIQUE 부재 ⑥ 4문서 인쇄 본문 통합(P4.5 사안).
...
*문서 버전: v2.4 · P0~P3 완료 + **P4.0~P4.4 완료·종결**(2026-07-17) — 발번·날짜 전면 KST화 / 마스터 감사 / `000_baseline.sql` 재구축 복원 / Vitest(테스트 137) / `stock_movements` 추가전용 원장·권한 봉인·역분개(D1·D2·D3) / 입고 참조생성·`GR_IN` 전기·잔량 뷰·부분입고·세대 도장·잔량 소비 가드(C5) / 출고 참조생성·`DLV_OUT` 전기·부분출고·마이너스 경고 후 허용·거래명세서 인쇄(B8) / P4.3f uom 폴백 정정('PCS' 발명 금지) / **P4.4 선적 화물 라인(diff-upsert·동시성 베이스라인·소비 가드 3겹) + 당사자 스냅샷(인쇄 불변) + S/I 인쇄(단위별·포장유형별 TOTAL·금액 부재) — 스팟체크는 오너 직권 지시로 생략(Playwright 6시나리오 갈음)**. 다음: P4.4h 봉인 하드닝(감사표 기반, 아키텍트 결정) · P4.5 무역서류(CI/PL) → P4.6 문서 흐름 추적*
