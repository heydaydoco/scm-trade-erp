...
- E5. 컨테이너 적입(CBM/적입계획), 쉬핑마크, VGM ✅ *구현(P5.2 — shipment_containers 컨테이너 실측(번호·타입·씰·VGM) + shipment_container_allocations 라인 포장수 배분. 적입 지표(배분 포장수 합·비례 G.W./CBM·용적률)는 전부 **파생 계산·표시 전용·저장 금지**. 쉬핑마크는 P4.4/P4.5c0 기구현. **P5.3 — 적입이 CI/PL 발행 스냅샷으로 동결**(문서 스코프·번호/타입/씰 3필드+파생 수치, **VGM은 고객 문서에서 제외=정량 유출 차단**, VGM은 S/I 소관). 미배정 번호 = TBA)* `[P5]`
...
- **P4 — 재고 원장 + 출고/입고 + 서류생성 + 문서흐름**: 재고 코어, Delivery/GR(참조생성), CI/PL 생성기, 흐름 추적 화면 ← **여기까지가 진짜 ERP의 1차 완성**
  ...
  - ✅ **P5.3 CI/PL 적입 스냅샷 + TBA + 검증 인프라 완료·종결** (2026-07-24, 커밋 `2906397`(a 순수-반올림/TBA)→`56f0ab0`(b 순수-헤더3상태/매퍼)→`5489fcf`(f 1차 마이그레이션·containers_snapshot+RPC)→`47d5f0d`(c 읽기경로)→`48314de`(d 화면)→`1c1a6d2`(e 검증인프라)→`7e5f9ed`(g §4 VGM 제외)→`3bd31f6`(h nit 반영)→`b746ce0`(i 2차 마이그레이션·VGM 제거)→`9607332`(j 델타 반영)) — **P5.2 백로그 2종(적입 스냅샷·TBA) 소진.**
    - **범위/판정(P1~P4)**: P1 발행 RPC `save_trade_document` = **CREATE OR REPLACE 단독**(DROP 금지·시그니처 6인자 무변경·grant 무접촉·신규 테이블0·신규 RPC0 → **봉인 기준선 34 불변**). P2 스냅샷 스코프 = **문서 포함 라인 기준**(포함 라인에 배분 1건+ 걸린 컨테이너만·각 컨테이너엔 그 문서 라인 배분만·배분0/타 라인만 컨테이너 제외 = 혼합 선적 타 고객 유출 차단). P3 발행 스냅샷 = **발행 시점 사실의 동결**(서버가 문서 스코프 파생 수치까지 계산·저장, 인쇄는 계산0·읽기만). P4 TBA = 컨테이너 번호 미확정 전용(null 또는 trim()=='' → "TBA" 대문자 3자, 상수는 규칙함수 `displayContainerNo` 한 곳·표시층 전용·DB 무접촉. 배분 미실시 '-'는 별개 사실). 적용 = S/I 소급 + PL + CI/PL 헤더.
    - **설계(D1~D13)**: D1 `trade_documents.containers_snapshot jsonb` 1개 신설. D2 **NULL 허용·default 없음 → 3상태**(NULL=P5.3 이전 발행→스칼라 폴백 / 빈 구조=적입 0건→항목 생략·폴백 금지 / 값=적입 있음). D3 구조 `{containers:[{containerNo,containerType,sealNo,allocations:[{shipmentLineId,allocatedPackageCount}],packageCount,grossWeightKg,cbm,gwIncomplete,cbmIncomplete}], totals:{...}}` — **개정 2호로 vgmKg 제외(3필드)**. 컨테이너/배분 정렬 = created_at,id. D4 스냅샷 원천 = **서버 재조회**(클라 payload 추가 금지). D6 적입 2테이블 발행잠금 **비승격**(화인 동급 계승). D9 CONTAINERS 표 = **PL 전용**(CI 표 없음)·헤더 Container No.는 3상태 폴백 규칙(`documentContainerNoLabel`)·`container_no` 스칼라 사장 계보 유지(RPC 신규 주입 금지). D10 S/I↔PL 표 마크업 공용화 안 함(레이아웃 이관 백로그 유지)·표시 규칙만 순수함수 단일화. D11 `displayContainerNo` 순수함수+Vitest. D12 `mapContainers` export·안전 생략 계약(null/비객체→null·키 결손 던지지 않음). D13 S/I 수정 = containerNo 표시 지점만(레이아웃 무접촉).
    - **개정 1호(반올림)**: 비례 몫 반올림 의미론 = **몫의 참값 기준 half away from zero·소수 6자리·비음수**. TS(`containerLogic`)·SQL 양측이 **정확 십진 산술**로 구현해 동치를 **달성**한다 — `round6`(double 경유) ≡ SQL `round(x,6)` 은 **거짓**(착수 전 실측 40만 건 중 273건 tie 불일치). P4.5 `mulRound2`/`allocRound2` 계보의 6자리판(`scaledOrNull`→BigInt 스케일 정수→나머지 비교). SQL은 `div(v*p*10^6*2+w, w*2)*1e-6`(정수 스케일·나눗셈 무근사). **합 경로는 `round6` 유지**(피가산 항이 전부 6자리 십진 → tie 불가). **P5.2 표시값 개정 승인**: S/I 표시값이 tie 케이스 마지막 자리 ±1 변동(참값 교정 = S/I·PL·SQL 삼면 일치). 회귀 4건 성문화(83.357×20/64→26.049063 / 88.450588×25/40→55.281618 / 2355.499×75/240→736.093438 / 16.487×49/560→1.442613).
    - **개정 2호(§4 VGM 고객 문서 제외)**: 컨테이너 번호·타입·씰 = 공유 물리 사실(수취·통관 필수·정량 유출 없음) → 유지. **VGM = 컨테이너 총질량 = 고객 문서(CI/PL) 스냅샷·인쇄에서 전면 제외**(공동적입 시 타 고객 물량 합산 정량 → 유출). **데이터 층부터 제거**(인쇄만 숨기기 금지 — `trade_documents` 는 select 봉인 개방이라 데이터 잔존=유출). VGM 의 자리는 **S/I**(선사 방향·무접촉). 조건부(전용/공동 판정) 기각 = 배분 기록 불완전 가능(빈 컨테이너·부분 배분 허용) → 균일 제외가 완결. **잔여 경계**: 공동적입의 '존재'는 번호 공유로 드러남 = 물리 공유 사실로 허용·차단 대상은 **정량 정보뿐**.
    - **P3 경계 선언(SPEC v3.0 명문화)**: P5.2 판정 ④(적입 지표 **파생 저장 금지**)는 **라이브 테이블·화면 규율**로 그대로 유지 — 발행 스냅샷(발행 시점 사실 동결, D3 할인 배분과 같은 계보)은 그 위반이 아니라 **별개 계보**다.
    - **D8 소멸**: '미배분 전량 표기 시 면책 문구'(D8) = P2 스코프 규칙으로 '미배분 전량 표기' 경로 자체가 없어져 소멸.
    - **C 절(검증 인프라)**: C2 `scripts/verify_seal.sql` 기대 객체수 34 갱신·RPC 목록에 P5.1/P5.2 신설분 편입(드리프트 교정). C3 마이그레이션 사전점검 = **헤더 '실행 전제' 방식 통일**(별도 preflight 파일 신설 금지). C4 `scripts/checks.sql` 무역서류·적입 검산 5항목(㉮ 라인합↔소계 · ㉯ 총액↔소계−할인 · ㉰ 활성 문서↔취소 선적 · ㉱ 적입 스냅샷 자기일관[컨테이너 내부 배분합 + 총계 2축] · ㉲ 적입 과배분 리포트).
    - **검증**: Vitest 336→**376**(+40) · tsc0 · eslint0 · next build 성공 · 2회전 적대검증(5차원 변경분 → 개정 2호 3차원, 확정 결함 blocking/major **0**·nit/minor 반영). SQL≡TS 동치 = BigInt 시뮬레이션 10만 건 + `containerMetrics` 대조 3만 건 불일치 0. E2E **21단언 전항목 PASS**(로컬 :3100 임시 Seller) + 라이브 배포 스모크(VGM 열 제거 확인).
    - **스팟체크 귀속**: **오너 직권 위임 대행 + 오너 직접 확인 병행**(오너 "니가 직접 다 체크해봐" → 클코 브라우저 렌더 육안+자동 단언). 아키텍트 조건부 사전 승인(3/3→push)으로 push→배포→E2E.
    - **인시던트 기록(정리 SQL 이중 실행)**: 교정본 정리 SQL 이 07:43:49 **단일 트랜잭션 1회 성공 실행**(감사 계보 9행 동일 마이크로초 + 비감사 자식 cascade)했으나 성공 **NOTICE 가 Supabase 표시 누락** → 오너 재실행 → **2차 실행은 3중 일치 가드가 정상 차단(P0001)·삭제 0행**(무손상). 읽기전용 포렌식(audit_log DELETE 클러스터 단일·계보 밖 0·이중 삭제 없음)으로 확정. 규율 개정 2종: 성공 보고=최종 SELECT 표(RAISE NOTICE 금지·리터럴 % 는 %% 이스케이프) / 오너 Run 1회·재실행은 지시 시만.
    - **백로그 갱신**: P5.2 백로그 2종(적입 스냅샷·TBA) **완료 처리**. 신규 백로그 = **스냅샷↔라이브 적입 괴리 표시**(발행 후 라이브 적입이 갱신돼도 문서 스냅샷은 불변 = 정상이나, 화면에서 괴리를 알릴지는 별도 승인). 유지: 기존 인쇄 5벌 공용 레이아웃 이관 · D7 Seller 실값(실발행 전 필수·**재촉 금지**) · SQL 산식 pgTAP 골든 픽스처.
